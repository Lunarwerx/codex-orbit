...
;(() => {
  try {
    ...
    let liveStatus=new Map();   // conversationId -> running|waiting|failed, fed by the event-stream listener
    function statusOf(t){
      const ls=liveStatus.get(String(t&&t.id))||liveStatus.get(String(t&&t.__id));
      if(ls) return ls;
      const raw=(t&&t.raw)||t||{};
      try{
    ...
    const DOT={running:"var(--vscode-charts-blue,#3b82f6)",waiting:"var(--vscode-charts-orange,#f5a524)",failed:"var(--vscode-charts-red,#e5484d)",review:"var(--vscode-charts-green,#30a46c)",idle:"#6e6e6e"};
    ...
    try{ window.__codexOrbitStatusHook=function(id,state){ try{ const k=String(id),s=deriveStatus(state); if(s&&s!=="idle"){ if(liveStatus.get(k)!==s){ liveStatus.set(k,s); scheduleRender(); } } else if(liveStatus.has(k)){ liveStatus.delete(k); scheduleRender(); } }catch{} }; }catch{}
...
.coxSpin{width:9px;height:9px;flex:0 0 9px;border-radius:50%;border:1.6px solid var(--vscode-descriptionForeground,#555);border-top-color:var(--vscode-charts-blue,#3b82f6);animation:coxspin .7s linear infinite;align-self:flex-start;margin-top:4px}
...
.coxDot{width:7px;height:7px;flex:0 0 7px;border-radius:50%;background:#6e6e6e;align-self:flex-start;margin-top:5px}
...
# SAFE: if the (exact-5519) anchor ever drifts it simply doesn't apply (status stays
# best-effort), it can NEVER break Codex. Status is otherwise unreachable from a window
# listener — it rides the app-server IPC bridge into Jotai atoms, never onto `window`.
STATUS_ANCHOR = "updateConversationState(e,t){let n=this.getConversation(e);if(n==null)return;let r=Nn(n,t);this.applyConversationState(e,r)}"
STATUS_INJECT = "updateConversationState(e,t){let n=this.getConversation(e);if(n==null)return;let r=Nn(n,t);this.applyConversationState(e,r);try{if(window.__codexOrbitStatusHook)window.__codexOrbitStatusHook(e,r)}catch(_co){}}"


def expose_status_stream(wv):
    """Tee Codex's per-conversation status into window.__codexOrbitStatusHook so the sidebar
    can show live Thinking/Question/Failed. See the STATUS_ANCHOR note above."""
    for f in wv.glob("*.js"):
        if f.name.endswith(".map"):
            continue
        text = f.read_text(encoding="utf-8", errors="ignore")
        ...
        if STATUS_ANCHOR in text:
            f.write_text(text.replace(STATUS_ANCHOR, STATUS_INJECT, 1), encoding="utf-8", newline="")
