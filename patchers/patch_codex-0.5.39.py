...
;(() => {
  try {
    ...
    let usageStatus=null, usageRows=[], usageLabels=[], usageUpdatedAt=0;
    ...
    window.addEventListener("message",(e)=>{ try{ ingest(e.data); }catch{} try{ ingestRoots(e.data); }catch{} try{ ingestUsage(e.data); }catch{} },true);
    try{
...
`;
    ...
    function showMenu(x,y,t){
      ...
      // gone; Copy diagnostics moved to the Settings gear menu.
    ...
    function usageBuckets(){ let b=collectBuckets(usageRows); if(!b.length) b=collectBuckets(usageStatus); return b.length?b:bucketsFromLabels(usageLabels); }
    ...
    function showUsageMenu(btn){
      ...
      if(buckets.length){
        ...
        const rows=document.createElement("div"); rows.className="coxUsageRows";
        buckets.forEach((b)=>{ const r=document.createElement("div"); r.className="coxUsageRow"; const reset=b.resetText||relReset(b.resetAt)||shortClock(b.resetAt)||""; r.innerHTML='<strong></strong><span></span>'; r.querySelector("strong").textContent=b.window||b.name; r.querySelector("span").textContent=reset; rows.appendChild(r); });
        m.appendChild(rows);
      } else {
    ...
    function showInstructionsDialog(){
      ...
      const shade=document.createElement("div"); shade.className="coxModalShade";
      const d=document.createElement("div"); d.className="coxDialog"; d.setAttribute("role","dialog"); d.setAttribute("aria-modal","true");
      const h=document.createElement("div"); h.className="coxDialogTitle"; h.textContent="Custom instructions";
      const p=document.createElement("p"); p.className="coxDialogBody"; p.textContent="Codex reads AGENTS.md. Project instructions live in this workspace; personal defaults live in ~/.codex/AGENTS.md.";
      const a=document.createElement("div"); a.className="coxDialogActions";
      const close=document.createElement("button"); close.type="button"; close.className="coxDialogBtn coxCancel"; close.textContent="Close";
      const project=document.createElement("button"); project.type="button"; project.className="coxDialogBtn"; project.textContent="Project AGENTS.md";
      const global=document.createElement("button"); global.type="button"; global.className="coxDialogBtn primary"; global.textContent="Global AGENTS.md";
      a.append(close,project,global); d.append(h,p,a); shade.appendChild(d); document.body.appendChild(shade);
      const done=()=>closeOrbitDialog();
      const openAgents=(scope)=>{
        const hw=hostWorkspace(), root=(hw&&hw.r)||curRoot||"";
        const payload={scope};
        if(scope==="project"&&root) payload.path=root.replace(/[\\/]+$/,"")+(/\\/.test(root)?"\\":"/")+"AGENTS.md";
        if(!coxPost("codex-orbit-open-agents",payload)){
          const fallback=scope==="project"?(payload.path||"AGENTS.md"):"~/.codex/AGENTS.md";
          copyText(fallback); showOrbitInfo("Custom instructions","Copied "+fallback+" to the clipboard.");
        } else done();
      };
      shade.addEventListener("click",(ev)=>{ if(ev.target===shade) done(); });
      close.addEventListener("click",done);
      project.addEventListener("click",()=>openAgents("project"));
      global.addEventListener("click",()=>openAgents("global"));
      setTimeout(()=>{ try{ project.focus(); }catch{} },0);
    }
    ...
    function showSettingsMenu(btn){
      ...
      item("Account & usage",()=>showUsageMenu(shell.querySelector(".coxUsage")||btn));
      ...
      item("Copy diagnostics",()=>{ let o={}; try{ o=(window.codexOrbitDump&&window.codexOrbitDump({download:false}))||{}; }catch(e){} copyText(JSON.stringify(o,null,2)); });
      anchorMenu(m,btn);
    }
    // ---------- row hover actions ----------
...
USAGE_ROWS_RE = re.compile(
    ...
    r"let\s+([A-Za-z_$][\w$]*)=\[\],[\s\S]{0,5000}?additional_rate_limits[\s\S]{0,5000}?return )\3(\})"
)
...
def expose_usage_rows(wv):
    ...
    for f in wv.glob("*.js"):
        ...
        injected = (
            ...
            f"}}catch(_co){{}}return {rows}{m.group(4)}"
        )
        f.write_text(text[:m.start()] + injected + text[m.end():], encoding="utf-8", newline="")
...
AGENTS_OPEN_CASE = (
    ...
    'try{await iC.promises.access(o)}catch(i){let s=i;if("code"in s&&s.code==="ENOENT")'
    'await iC.promises.writeFile(o,n==="global"?"# Global Codex Instructions\\n\\n":"# Project Codex Instructions\\n\\n");else throw i}'
...
def inject_agents_opener(ext):
    ...
    if not host.exists():
        ...
        return False
    text = host.read_text(encoding="utf-8", errors="ignore")
    if "codex-orbit-open-agents" in text:
