...
;(() => {
  try {
    ...
    try{ window.__codexOrbitStatusHook=function(id,state){ try{ const k=uuidOf(id)||String(id),s=deriveStatus(state); if(s&&s!=="idle"){ if(liveStatus.get(k)!==s){ liveStatus.set(k,s); scheduleRender(); } } else if(liveStatus.has(k)){ liveStatus.delete(k); scheduleRender(); } }catch{} }; }catch{}
