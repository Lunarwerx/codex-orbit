...
;(() => {
  try {
    ...
    let usageStatus=null, usageLabels=[], usageUpdatedAt=0;
...
`;
    ...
    function usageBuckets(){ const b=collectBuckets(usageStatus); return b.length?b:bucketsFromLabels(usageLabels); }
    ...
    function showUsageMenu(btn){
      ...
      m.querySelector(".coxUsageFresh").addEventListener("click",(ev)=>{ ev.stopPropagation(); try{ if(window.__codexOrbitRateLimitStatus) setUsageStatus(window.__codexOrbitRateLimitStatus); if(window.__codexOrbitUsageLimits) setUsageLabels(window.__codexOrbitUsageLimits); coxPost("refresh-rate-limit-status",{}); }catch{} showUsageMenu(btn); });
    ...
    function showInstructionsDialog(){
      showOrbitInfo("Custom instructions","Codex uses AGENTS.md for durable instructions. Put shared project guidance in this repository's AGENTS.md, or personal defaults in ~/.codex/AGENTS.md. There is no Codex.md file to open here.");
    }
    function showThemeDialog(){
      showOrbitInfo("Color theme","The Codex webview follows your VS Code theme. The Codex TUI has its own /theme picker, but this sidebar does not expose a separate Codex color-theme picker.");
    }
    ...
    function showSettingsMenu(btn){
      ...
      item("Color theme",()=>showThemeDialog());
