...
`;
    ...
    function ensureShell(){
      ...
      // Event delegation on the persistent .coxList: row <button>s (and group
      // headers) are rebuilt on every render(), so per-element listeners get torn
      // out mid-click — mousedown on the old button, mouseup on its replacement, so
      // the browser fires NO `click` and the row appears dead. The container survives
      // renders and `click` still dispatches to it (common ancestor of the swapped
      // targets), so routing here is immune to the re-render race. The right-click
      // menu always worked precisely because it lives on document.body, not in here.
