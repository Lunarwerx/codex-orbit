...
;(() => {
  try {
    ...
    let shell=null, collapsed=false, search="", groupState=Object.create(null), dataThreads=[], pins=new Set(), stars=new Set();
    ...
    // The current workspace's project. Priority: the open/active chat's project;
    // else a workspace label scraped from Codex's own UI; else null -> show all
    // (never lock onto a wrong guess like "most recent thread globally").
    function workspaceFromDom(projects){
      if(!projects.length) return null;
      const byLower=new Map(projects.map((p)=>[p.toLowerCase(),p]));
      const els=document.querySelectorAll("button,[role='button'],[class*='workspace' i],[class*='project' i],[class*='root' i]");
      for(const el of els){ if(el.closest&&el.closest(".coxSidebar")) continue; if(el.children&&el.children.length>2) continue; const tx=clean(el.textContent); if(tx){ const hit=byLower.get(tx.toLowerCase()); if(hit) return hit; } }
      return null;
    }
    function currentProject(){
      const list=allThreads();
      const a=list.find((t)=>t.current); if(a) return a.project;
      return workspaceFromDom([...new Set(list.map((t)=>t.project))]);
    }
