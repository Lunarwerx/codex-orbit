...
;(() => {
  try {
    ...
    let filterState={types:[],age:0,hideUntitled:false};
...
.coxActs{display:none;position:absolute;right:5px;top:50%;transform:translateY(-50%);align-items:center;gap:1px;background:var(--vscode-sideBar-background,#1e1e1e);border-radius:5px;padding:1px;box-shadow:-6px 0 6px var(--vscode-sideBar-background,#1e1e1e)}
...
.coxMenuTog{display:flex;align-items:center;gap:8px;justify-content:space-between}
.coxTog{width:26px;height:15px;border-radius:8px;background:var(--vscode-input-background,#3a3a3a);position:relative;flex:0 0 26px}
.coxTog::after{content:"";position:absolute;top:2px;left:2px;width:11px;height:11px;border-radius:50%;background:#bbb;transition:left .12s}
.coxMenuItem.on .coxTog{background:var(--vscode-focusBorder,#3b82f6)}
.coxMenuItem.on .coxTog::after{left:13px;background:#fff}
...
`;
    ...
    function showFilterMenu(btn){
      ...
      sep(); head("Hide");
      const hu=document.createElement("button"); hu.type="button"; hu.className="coxMenuItem coxCheck"+(f.hideUntitled?" on":""); hu.textContent="Untitled chats"; hu.addEventListener("click",(ev)=>{ ev.stopPropagation(); f.hideUntitled=!f.hideUntitled; hu.classList.toggle("on"); saveFilter(); render(); }); m.appendChild(hu);
      sep();
      const clr=document.createElement("button"); clr.type="button"; clr.className="coxMenuItem"; clr.textContent="Clear all"; clr.addEventListener("click",(ev)=>{ ev.stopPropagation(); filterState={types:[],age:0,hideUntitled:false}; saveFilter(); render(); closeMenu(); }); m.appendChild(clr);
      anchorMenu(m,btn);
    }
    function filterActive(){ const f=filterState; return (f.types&&f.types.length)||f.age||f.hideUntitled; }
    ...
    function showSettingsMenu(btn){
      ...
      const toggle=(label,id)=>{ const b=document.createElement("button"); b.type="button"; b.className="coxMenuItem coxMenuTog"; const s=document.createElement("span"); s.textContent=label; const g=document.createElement("span"); g.className="coxTog"; b.append(s,g); b.addEventListener("click",(ev)=>{ ev.stopPropagation(); b.classList.toggle("on"); runCmd(id); }); m.appendChild(b); };
      toggle("YOLO mode","yoloMode");
      toggle("Chat preview header","chatPreviewHeader");
      const sep=()=>{ const d=document.createElement("div"); d.className="coxMenuSep"; m.appendChild(d); }; sep();
    ...
    function archiveThread(t){
      // Codex's own command (our history patch uses the same). Post both envelope shapes
      // to maximise acceptance, plus an optimistic local hide so OUR list updates at once.
      coxPost("archive-conversation",{conversationId:t.id});
      coxPost("archive-conversation",{params:{conversationId:t.id}});
      archived.add(t.id); saveSet(ARCH_KEY,archived); render();
    }
    ...
    function addRow(parent,t){
      ...
      const col=colors.get(t.id); if(col) b.style.boxShadow="inset 3px 0 0 "+(colorOf(col)||col);
      ...
      mk("archive","Archive",IC.archive,false);
      b.append(dot,mark,tt,acts);
    ...
    function applyFilter(rows){
      ...
      if(f.hideUntitled){ out=out.filter((t)=>{ const ti=clean(t.title).replace(/^⭐\s*/,"").replace(/^📌\s*/,""); return ti&&ti!=="Untitled"&&ti!=="New chat"&&ti!=="Untitled chat"; }); }
      return out;
    }
    function render(){
      ...
      rows=rows.filter((t)=>!archived.has(t.id)&&!(t.raw&&t.raw.archived===true));   // drop archived (ours + Codex's)
      rows=applyFilter(rows);
      const fb=side.querySelector(".coxFilter"); if(fb) fb.classList.toggle("on",!!filterActive());
      list.textContent="";
      if(!rows.length){ const e=document.createElement("div"); e.className="coxEmpty"; e.textContent=search?"No matching chats.":(filterActive()?"No chats match the filter.":"No chats found yet."); list.appendChild(e); return; }
