...
.coxSidebar{position:fixed;top:0;right:0;bottom:0;width:var(--cox-open);z-index:2;display:flex;flex-direction:column;background:var(--vscode-sideBar-background,#171717);color:var(--vscode-sideBar-foreground,var(--vscode-foreground,#d4d4d4));border-left:1px solid var(--vscode-sideBar-border,#2b2b2b);font-family:var(--vscode-font-family,system-ui,sans-serif);font-size:12px;transition:width .14s ease}
...
.coxSideClosed .coxSidebar{left:auto;right:0;top:0;bottom:auto;width:auto;height:auto;border:0;border-left:1px solid var(--vscode-sideBar-border,#2b2b2b);border-bottom:1px solid var(--vscode-sideBar-border,#2b2b2b);border-radius:0 0 0 8px;box-shadow:0 4px 14px rgba(0,0,0,.35);transition:none}
...
`;
    ...
    function clickStrong(el){
      const target=(el&&el.closest&&el.closest("a,button,[role='button'],[tabindex]"))||el;
      if(!target) return false;
      try{ target.scrollIntoView({block:"nearest",inline:"nearest"}); }catch{}
      const ev=(name,Ctor,extra)=>{ try{ target.dispatchEvent(new Ctor(name,{bubbles:true,cancelable:true,view:window,...(extra||{})})); }catch{} };
    ...
    function openThread(t){
      ...
      if(t.element&&clickStrong(t.element)) return;
      try{ const row=document.querySelector(`[data-app-action-sidebar-thread-id="${(window.CSS&&CSS.escape)?CSS.escape(t.id):t.id}"]`); if(row&&clickStrong(row)) return; }catch{}
      // 2) Bridge a wrong/stale id by matching a native row on title.
      try{ const want=titleKey(t.title); for(const el of document.querySelectorAll(A.row)){ if(titleKey(el.getAttribute(A.title)||el.textContent)===want&&clickStrong(el)) return; } }catch{}
      // 3) React Router (unstable_HistoryRouter): /local/<id> for local, /remote/<id> for a remote host.
      const remote=(t.kind&&t.kind.toLowerCase()==="remote")||(t.hostId&&t.hostId.toLowerCase()!=="local");
      navTo((remote?"/remote/":"/local/")+encodeURIComponent(t.id));
    }
