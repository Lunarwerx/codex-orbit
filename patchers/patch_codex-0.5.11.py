...
;(() => {
  try {
    ...
    const IC={
      ...
      gear:'<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.35" stroke-linecap="round"><path d="M3 4.5h10M3 8h10M3 11.5h10"/><circle cx="6" cy="4.5" r="1.25" fill="var(--vscode-sideBar-background,#171717)"/><circle cx="10" cy="8" r="1.25" fill="var(--vscode-sideBar-background,#171717)"/><circle cx="7" cy="11.5" r="1.25" fill="var(--vscode-sideBar-background,#171717)"/></svg>',
...
`;
    ...
    function ensureShell(){
      ...
      shell.querySelector(".coxSettings").addEventListener("click",()=>{ openSettings(); });
    ...
    function openSettings(){
      appPost("show-settings",{});
      hostPost("show-settings",{});
      appPost("navigate-to-route",{path:"/settings"});
      hostPost("navigate-to-route",{path:"/settings"});
      setTimeout(()=>{ if(location.pathname.indexOf("/settings")<0) navTo("/settings"); },80);
    }
