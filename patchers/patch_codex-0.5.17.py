...
;(() => {
  try {
    ...
    const rowMap=new WeakMap();  // row <button> -> thread; delegated .coxList handlers read this so clicks survive the list re-render
    ...
    function inCurrent(t){
      ...
      if(curRoot){
        ...
        // No cwd on this thread (rare): only then fall back to an exact project name.
        return curLabel ? clean(t.project).toLowerCase()===curLabel : false;
      }
    ...
    function currentRows(rows,nativeRows){
      // The authoritative current workspace is Codex's active-workspace-roots[0] — a
      // real path. Set it FIRST, before any label/native heuristic, so the cwd path
      // (not a folder name) is what decides. Only if roots haven't arrived yet do we
      // fall back to the OPEN chat's own cwd; we do NOT let an opened chat from another
      // workspace hijack the filter away from the VS Code workspace.
      if(activeRoots[0]) setCurrent(activeRoots[0],lastSeg(activeRoots[0]));
      else { const open=rows.find((t)=>t.current||t.active); if(open&&open.cwd) setCurrent(open.cwd,open.project); }
      // Known workspace path -> filter strictly by cwd. This is what stops other
      // workspaces' chats from leaking into the list.
      if(curRoot){
        try{window.__codexOrbitCurrentProject=curLabel||lastSeg(curRoot)||null;}catch{}
        const f=rows.filter(inCurrent);
        if(f.length) return f;
      }
      // Path not known yet (host roots message not seen): scope to Codex's native
      // sidebar (which it renders for the current workspace) by precise id/title.
      const nativeScope=nativeScopedRows(rows,nativeRows);
      if(nativeScope) return nativeScope;
      // Still nothing: show only the open chat rather than leaking every project.
      if(!curRoot&&!curLabel){
...
`;
    ...
    function addRow(parent,t){
      ...
      b.append(mark,tt);
      rowMap.set(b,t);   // open + right-click are routed by the delegated .coxList handlers
