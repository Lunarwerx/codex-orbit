...
;(() => {
  try {
    ...
    function currentRows(rows){
      ...
      return f.length?f:rows; // last-resort guard against a confusing empty list
    }
...
`;
    ...
    function openThread(t){
      ...
      try{ const row=document.querySelector(`[data-app-action-sidebar-thread-id="${(window.CSS&&CSS.escape)?CSS.escape(t.id):t.id}"]`); if(row){ (row.closest("a,button")||row).click(); return; } }catch{}
      // 2) Bridge a wrong/stale id by matching a native row on title.
      try{ const want=titleKey(t.title); for(const el of document.querySelectorAll(A.row)){ if(titleKey(el.getAttribute(A.title)||el.textContent)===want){ el.click(); return; } } }catch{}
    ...
    window.codexOrbitDump=function(){
      ...
      const data={
        ...
        filteredCount:currentRows(all).length,
