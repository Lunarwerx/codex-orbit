...
;(() => {
  try {
    ...
    function currentRows(rows,nativeRows){
      ...
      // project name). This is what stops other workspaces' chats from leaking in.
      if(curRoot||curLabel){
      ...
      // Truly blind (no open chat, no roots, no active flag, no native rows): prefer
      // the active chat; otherwise show a few rows so the sidebar isn't barren.
