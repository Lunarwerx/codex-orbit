...
;(() => {
  try {
    ...
    // Merge a freshly-normalized record over the one we already hold WITHOUT losing
    // information. Codex re-sends the same chat in lighter "recent tasks" / notification
    // payloads that omit cwd — so projectOf() falls back to "Other". A blind map.set()
    // let that SPARSE record clobber the RICH one, stripping the chat's folder so it no
    // longer matched the current-workspace filter and silently vanished from the list
    // (the "appears then disappears" bug: 1925 threads ingested, only the open chat — the
    // one Codex keeps re-sending in full — survived the filter). Prefer the non-empty /
    // non-default value for every identity field; ts only ever moves forward.
    function mergeThread(prev,next){
      if(!prev) return next;
      const cwd=next.cwd||prev.cwd||"";
      // A record that carries a cwd has the trustworthy project (derived from it). When
      // next omits cwd, keep prev's cwd-derived project instead of its "Other" fallback.
      const project=next.cwd?next.project:(prev.cwd?prev.project:(next.project&&next.project!=="Other"?next.project:(prev.project||next.project)));
      return Object.assign({},prev,next,{
        title:(next.title&&next.title!=="Untitled")?next.title:(prev.title||next.title||"Untitled"),
        cwd,project,
        hostId:next.hostId||prev.hostId||"",
        kind:next.kind||prev.kind,
        ts:Math.max(next.ts||0,prev.ts||0),
    ...
    function ingest(payload){
      try{
        ...
        let changed=false;
        for(const raw of arr){ const n=normalizeRaw(raw); if(n){ map.set(n.__id,mergeThread(map.get(n.__id),n)); changed=true; } }
    ...
    function rememberStatusThread(id,state,st){
      ...
      const hw=hostWorkspace(); if(hw) setFromHost(hw);
    ...
    // CUR_KEY is a FLAT localStorage key, and VS Code webview localStorage is scoped per
    // extension origin and SHARED across windows/workspaces. So a home learned in workspace
    // A used to reload under workspace B and strict-filter B to EMPTY (sticky across reloads
    // — the "his chats disappear" case, worst on a Codex build where our <meta> anchor was
    // missed so nothing re-corrects it). Fix: namespace the persisted home by a workspace
    // id (the authoritative host meta), and only reload it when it belongs to THIS workspace.
    function wsKeyNow(){ const hw=hostWorkspace(); if(!hw) return ""; return normPath(hw.r||"")||clean(hw.l||"").toLowerCase(); }
    function persistCur(){ try{ localStorage.setItem(CUR_KEY,JSON.stringify({r:curRoot,l:curLabel,wid:wsKeyNow()||curRoot||curLabel})); }catch{} }
    try{ const c=JSON.parse(localStorage.getItem(CUR_KEY)||"{}"); const wid=wsKeyNow();
         if(c.wid&&wid&&c.wid===wid){ curRoot=c.r||""; curLabel=c.l||""; }
    }catch{}
    ...
    // AUTHORITATIVE host meta: REPLACE both fields (clearing the half the meta omits) so a
    // stale label/root can never linger across a workspace switch. Strictly tightening — it
    // only ever narrows home to the real VS Code workspace; it never widens matching.
    function setFromHost(hw){
      if(!hw) return;
      const r=normPath(hw.r||""), l=clean(hw.l||"").toLowerCase();
      let ch=false;
      if(r){ const nl=l||lastSeg(r); if(r!==curRoot){curRoot=r;ch=true;} if(nl!==curLabel){curLabel=nl;ch=true;} }
      else if(l){ if(curRoot){curRoot="";ch=true;} if(l!==curLabel){curLabel=l;ch=true;} }
      if(ch) persistCur();
    }
    ...
    function currentRows(rows,nativeRows){
      ...
      //  Adopt it as home ONLY when at least one visible row actually belongs to it — an
      //  async RPC reply must never redefine home to a root that owns zero rows and thereby
      //  empty a populated list. Strictly narrowing, so it cannot leak.
      if(!curRoot&&!curLabel&&activeRoots[0]){
        const rt=normPath(activeRoots[0]), seg=lastSeg(activeRoots[0]);
        const corrob=rows.some((t)=>{ const c=normPath(t.cwd); if(c) return c===rt||c.indexOf(rt+"/")===0; const pj=clean(t.project).toLowerCase(); return !!pj&&pj===seg; });
        if(corrob) setCurrent(activeRoots[0],seg);
      }
      ...
      if(curRoot||curLabel){
        ...
        const scoped=rows.filter(inCurrent);
        if(scoped.length) return scoped;
        // Home is KNOWN but the strict filter matched NOTHING. That is the disappear bug:
        // an fsPath-vs-wire-cwd topology mismatch, a stale persisted root on a missed-anchor
        // build, or cwd-less rows whose label != home. Do NOT blank a list Codex itself is
        // showing, and do NOT fall through to the blind cap below. Re-scope ONLY via
        // this-workspace-bounded signals: (a) native DOM id/title intersection (inherently
        // THIS window's sidebar), then (b) an EXACT home-label match. Both are leak-safe (no
        // fuzzy/shared label, no first-8). If even those are empty, stay empty — preserving
        // the deliberate empty-when-known invariant and the anti-leak guarantee.
        const ns=nativeScopedRows(rows,nativeRows);
        if(ns&&ns.length) return ns;
        const lbl=curLabel||lastSeg(curRoot);
        if(lbl){ const byLabel=rows.filter((t)=>clean(t.project).toLowerCase()===lbl); if(byLabel.length) return byLabel; }
        return scoped; // genuinely empty -> stay empty
      }
    ...
    function filterNativeTaskLists(scopedRows){
      try{
        document.querySelectorAll(".coxNativeTaskHidden").forEach((el)=>{ try{ el.classList.remove("coxNativeTaskHidden"); }catch{} });
        const allowed=new Set((scopedRows||[]).map((t)=>threadTitleKey(t.title)).filter(Boolean));
        if(!allowed.size) return;
        const safeHide=(el,panel)=>{
          try{
            if(!el||el===panel||el===document.body||el===document.documentElement) return;
            if(shell&&(el===shell||el.contains(shell)||shell.contains(el))) return;
            if(el.querySelector&&el.querySelector(".coxSidebar")) return;
            if(clean(el.textContent).length>220) return;
            el.classList.add("coxNativeTaskHidden");
          }catch{}
        };
        ...
        for(const h of heads){
          ...
          if(!panel) continue;
          if(shell&&panel.contains(shell)) continue;
          ...
          for(const el of viewAlls){
            let target=el, cur=el.parentElement;
            while(cur&&cur!==panel){
              const tx=clean(cur.textContent);
              if(/^View all\b/i.test(tx)&&tx.length<80){ target=cur; cur=cur.parentElement; continue; }
              break;
            }
            safeHide(target,panel);
          }
          ...
          for(const el of candidates){
            ...
            if(!!key&&!allowed.has(key)) safeHide(el,panel);
          }
        }
      }catch{}
    }
...
def patch_webview_js(helper, patcher_version="") -> bool:
    """Make the patched Codex DETECTABLE by the Orbit launcher, carry the channel tag, and
    satisfy the launcher's OTA-accept marker.

    THREE jobs, all dictated by the shared launcher (pulled from claude-code-orbit):
    1. OTA-ACCEPT MARKER: the launcher's OTA loader accepts a fetched patcher only if its
       SOURCE contains the literal string `def patch_webview_js`. Keep this function name.
       (Our real injector is copy_patched_assets; the launcher's marker is Claude-named.)
    2. DETECTION MARKER FILE — THE FIX for "no Orbit patches yet": the launcher's
       detectState()/readInstalledPatcherVersion() decide "patched vs stock" by reading
       <ext>/webview/index.js and checking for `ccPatchSettingsBtn`/`ccPatchSessionItem`,
       then ccPatchBuildVersion + ccPatchChannel. Those are the CLAUDE patcher's class
       names at the CLAUDE webview path. Codex's webview is webview/assets/*.js using cox*
       classes, so without this file the launcher reports "no Orbit patches yet" no matter
       how many times you install. We write a tiny INERT webview/index.js carrying exactly
       the tokens the launcher greps for. Codex never loads index.js (clean Codex has no
       such file) — it is detection-only and safe.
    3. CHANNEL TAG: also append the markers to the injected chunk (belt-and-suspenders).

    Idempotent. Proper fix (launcher detecting Codex's own marker) belongs UPSTREAM."""
    ver = patcher_version if re.match(r"^\d+\.\d+", str(patcher_version or "")) else __version__
    wv_dir = helper.parent.parent   # <ext>/webview
    detect = (
        "/* Codex Orbit patch marker — read by the Orbit launcher's detectState(). Codex does "
        "NOT load this file; it exists only so the wrapper can detect the patch. The tokens "
        "ccPatchSettingsBtn / ccPatchSessionItem below are what the launcher greps for. */\n"
        f'var ccPatchBuildVersion="{ver}";\n'
        f'var ccPatchChannel="{ORBIT_CHANNEL}";\n'
    )
    (wv_dir / "index.js").write_text(detect, encoding="utf-8", newline="")
    # Belt-and-suspenders: also tag the injected chunk itself (appended, never prepended,
    # so it can't disturb the chunk's ES-module import order). Idempotent.
    text = helper.read_text(encoding="utf-8", errors="ignore")
    if "ccPatchChannel=" not in text:
        helper.write_text(text + f'\n;var ccPatchBuildVersion="{ver}";var ccPatchChannel="{ORBIT_CHANNEL}";',
...
def copy_patched_assets(extension_dir, patcher_version):
    ...
    patch_webview_js(helper, patcher_version)   # write webview/index.js detection marker + channel tag
